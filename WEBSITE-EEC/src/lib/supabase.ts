...
import { registrarEtapaAuth } from '../utils/auth-diagnosics'

let anonClientInstance: SupabaseClient | null = null

export function isSupabaseConfigured(): boolean { //Esta função verifica se o Supabase está disponível
    const env = getEnv()
...
export function createHonoSupabaseClient(c: Context) { // usuário normal autenticado pela autenticação web
    const env = getEnv()
...
export function getSupabaseAnonClient(): SupabaseClient | null { // operações públicas/anônimas do servidor
    if (!isSupabaseConfigured()) {
        return null
    }
